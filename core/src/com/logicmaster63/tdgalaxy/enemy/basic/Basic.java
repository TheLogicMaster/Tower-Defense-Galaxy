...
public class Basic extends Enemy{
    ...
    public Basic(Vector3 pos, double speeed, int hp, int health, int range, float coolDown, EnumSet<Types> types, EnumSet<Effects> effects, ModelInstance instance, btCollisionWorld world, IntMap<Entity> entities, boolean isTemplate, List<Vector3> path) {
        super(pos, speeed, hp, health, range, coolDown, types, effects, instance, new btCompoundShape(), world, entities, ATTACK_ANIMATION, ATTACK_OFFSET, isTemplate, path);
    ...
    public Basic(Vector3 position, ModelInstance instance, btCollisionWorld world, IntMap<Entity> entities, boolean isTemplate, List<Vector3> path) {
        this(position, SPEED, HP, HP, RANGE, COOLDOWN, TYPES, EnumSet.noneOf(Effects.class), instance, world, entities, isTemplate, path);
    }
